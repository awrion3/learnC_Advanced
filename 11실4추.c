...
int main(void) {
	...
	for (int i = 0; i < N - 1; i++) 	//버블 정렬 방식으로
		for (p = sp; p < sp + N - 1; p++) 	//구조체 배열을 포인터로 접근하여
			if (p->res <= (p + 1)->res) { 	//둘이 같거나 후자가 더 큰 총점인 경우, 오름차순 정렬
				if (p->res == (p + 1)->res) {	//둘이 같은 총점인 경우에는, 
					if (p->wt < (p + 1)->wt)	//필기 시험 점수가 더 높은 사람을 앞으로 이동
						swap(p, p + 1);		//해당 경우에 swap 함수 호출하여 두 구조체 원소를 교환
				}
				else						//후자가 더 큰 총점인 경우,
